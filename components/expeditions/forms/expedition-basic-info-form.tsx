...
interface ExpeditionBasicInfoFormProps {
  initialData: Partial<CreateExpeditionDto>;
  onSubmit: (data: Partial<CreateExpeditionDto>) => void;
  onCancel: () => void;
}

export function ExpeditionBasicInfoForm({
  initialData,
  onSubmit,
  onCancel,
  ...
  useEffect(() => {
    ...
    const fetchSellers = async () => {
      try {
        //@ts-ignore
        usersApiClient.getUsers({ userType: "SELLER" }).then((response) => {
        //@ts-ignore
