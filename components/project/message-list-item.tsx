...
const MessageListItem = ({ message }: MessageListItemProps) => {
  const { title, desc, active, hasnotifaction, notification_count, image } =
    message;
  return (
    <li className="flex gap-3 py-3 px-6 hover:bg-default-200">
      <div className="flex-1 flex gap-2">
        <div className="relative">
          ...
          <span
            className={cn(
              "absolute top-1 end-0 w-2 h-2 bg-secondary rounded-full ring-3 ring-default-200",
              { "bg-success": active }
            )}
          ></span>
        </div>
        <div>
          ...
          <div className="text-default-400 dark:text-default-600  text-xs mt-1">
            3 min ago
          </div>
        </div>
      </div>
